package LJVM;

import java.util.Scanner;
 
 
public class Lexer {
	/**
	 * 词法解析器：读入字符串（文本）对每个字符（单词）进行遍历并且为它们打上标签。再格式化输出
	 */
    public static final int  EOI = 0;         //end of file 结束符的标签（   end  ）
    public static final int  SEMI = 1;        //分号的标签；
    public static final int  PLUS = 2;        //加号的标签+
    public static final int  TIMES = 3;       //乘号的标签*
    public static final int  LP = 4;          //左括号（
    public static final int  RP = 5;           //右括号）
    public static final int  NUM_OR_ID = 6;    //数字的标签
    
    private int lookAhead = -1;              //当前分析到的标签，子字符串是哪一个标签，lookAhead=1说明当前要解析的标签是SEMI分号，boolean类型（return token == lookAhead;）
    
    public String yytext = "";            //用来分析字符串
    public int yyleng = 0;
    public int yylineno = 0;
    
    private String input_buffer = "";
    private String current = "";
    
    private boolean isAlnum(char c) {            //当前读取的是字母还是数字
    	if (Character.isAlphabetic(c) == true ||
    		    Character.isDigit(c) == true) {
    		return true;
    	}
    	
    	return false;
    }
    
    private int lex() {        //解析用户从控制台输入字符串
    
    	while (true) {
    		
    		while (current == "") {
    		    Scanner s = new Scanner(System.in);
    		    while (true) {
    		    	String line = s.nextLine();
    		    	if (line.equals("end")) {       //遇到end，就结束用户输入，并获取到字符串
    		    		break;
    		    	}
    		    	input_buffer += line;
    		    }
    		    s.close();
    		    
    		    if (input_buffer.length() == 0) {
    		    	current = "";
    		    	return EOI;
    		    }
    		    
    		    current = input_buffer;
    		    ++yylineno;
    		    current.trim();
    		}//while (current != "")
    		    
    		    for (int i = 0; i < current.length(); i++) {
    		    	
    		    	yyleng = 0;
    		    	yytext = current.substring(0, 1);         //每次读取一个字符并进行判断，yytext指向当期前字符
    		    	switch (current.charAt(i)) {
    		    	case ';': current = current.substring(1); return SEMI;
    		    	case '+': current = current.substring(1); return PLUS;
    		    	case '*': current = current.substring(1);return TIMES;
    		    	case '(': current = current.substring(1);return LP;
    		    	case ')': current = current.substring(1);return RP;
    		    	
    		    	case '\n':
    		    	case '\t':
    		    	case ' ': current = current.substring(1); break;
    		    	
    		    	default:
    		    		if (isAlnum(current.charAt(i)) == false) {
    		    			System.out.println("Ignoring illegal input: " + current.charAt(i));
    		    		}
    		    		else {
    		    			
    		    			while (isAlnum(current.charAt(i))) {
    		    				i++;
    		    				yyleng++;
    		    			} // while (isAlnum(current.charAt(i)))
    		    			
    		    			yytext = current.substring(0, yyleng);
    		    			current = current.substring(yyleng); 
    		    			return NUM_OR_ID;        //给字符串打上标签
    		    		}
    		    		
    		    		break;
    		    		
    		    	} //switch (current.charAt(i))
    		    }//  for (int i = 0; i < current.length(); i++) 
    		
    	}//while (true)	
    }//lex()
    
    public boolean match(int token) {
    	if (lookAhead == -1) {
    		lookAhead = lex();
    	}
    	
    	return token == lookAhead;
    }
    
    public void advance() {
    	lookAhead = lex();
    }
    
    public void runLexer() {
    	while (!match(EOI)) {
    		System.out.println("Token: " + token() + " ,Symbol: " + yytext );
    		advance();
    	}
    }
    
    private String token() {
    	String token = "";
    	switch (lookAhead) {
    	case EOI:
    		token = "EOI";
    		break;
    	case PLUS:
    		token = "PLUS";
    		break;
    	case TIMES:
    		token = "TIMES";
    		break;
    	case NUM_OR_ID:
    		token = "NUM_OR_ID";
    		break;
    	case SEMI:
    		token = "SEMI";
    		break;
    	case LP:
    		token = "LP";
    		break;
    	case RP:
    		token = "RP";
    		break;
    	}
    	
    	return token;
    }
}
